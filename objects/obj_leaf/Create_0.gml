...
mask_spr = spr_leaf_idle
idle_spr = spr_leaf_idle;
walk_spr = spr_leaf_walk;
run_spr = spr_leaf_run;
dash_spr = spr_leaf_dash;
jump_spr = spr_leaf_jump;
dash_jump_spr = spr_leaf_dash_air;
...
scr_state_free = function(){
 ...
 if (move_dir != 0){
    ...
    //Idle
    if abs(xspd) == 0 {sprite_index = idle_spr};
    //Walk
    ...
    if !on_ground 
	{
		...
		if (!on_ground)
		{
			...
			{
				// Animation ended
				image_index = image_number - 1 // Make sure it shows the last frame
				image_speed = 0;
			}
			image_speed = -1 
		}
	...
	//Run Jump
	if abs(xspd) >= move_spd[1] and !on_ground{sprite_index = dash_jump_spr}
